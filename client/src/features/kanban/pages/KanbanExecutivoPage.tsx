...
export function KanbanExecutivoPage() {
  ...
  const { data: projetos, isLoading: projetosLoading, refetch: refetchProjetos } = useProjetos('projeto_executivo');
  const moveProject = useMoveProject('projeto_executivo');
  const atribuirResponsavel = useAtribuirResponsavel('projeto_executivo');
  ...
  const etapas = etapasConfig?.projetoExecutivo || [];
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Kanban — Projeto Executivo</h1>
          <p className="text-muted-foreground">Arraste os projetos entre as etapas</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => refetchProjetos()} disabled={projetosLoading}>
      ...
      <KanbanBoard
        ...
        onProjetoSelect={setProjetoSelecionado}
      ...
      <ProjetoFormModal
        ...
        defaultTipoServico="projeto_executivo"
      />
    </div>
  );
}
